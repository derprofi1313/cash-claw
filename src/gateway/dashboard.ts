...
async loadRevenue(){try{const r=await this.fetchJson('/api/revenue');if(r.success!==false){Object.assign(this.revenue,r.data??r);this.$nextTick(()=>this.updateRevenueChart());}}catch{}},
...
handleWsEvent(msg){const evt=msg.event;const p=msg.payload||{};switch(evt){case'cycle_start':this.cycleCount=(p.cycle||this.cycleCount);this.addLog('info','AEL Zyklus #'+(p.cycle||'?')+' gestartet');break;case'cycle_end':this.actionsToday=p.actionsToday||this.actionsToday;this.addLog('info','Zyklus #'+(p.cycle||'?')+' fertig | '+p.tasksCompleted+' Tasks | $'+(p.costToday||0).toFixed(4));break;case'plan_complete':this.addLog('info','Plan: '+(p.tasks||0)+' Aufgaben');break;case'planning':this.addLog('info','Planung läuft...');break;case'task_start':this.addLog('tool_call','Task: '+(p.title||p.taskId||'?'));this.currentTask=p.title||null;break;case'task_complete':this.addLog(p.success?'info':'error',(p.success?'✅':'❌')+' Task '+p.taskId+' | $'+(p.costUsd||0).toFixed(4));if(p.success===false)this.currentTask=null;break;case'review_complete':this.addLog('info','Review: '+p.succeeded+' ok / '+p.failed+' fehler | $'+(p.costUsd||0).toFixed(4));break;case'cost_update':this.costToday=p.todayCost||0;break;case'tool_call':this.addLog('tool_call',p.tool+' ('+(p.duration||0)+'ms) '+(p.success?'✓':'✗'));break;case'agent_state':this.agentState=p.state||'idle';break;case'learning':this.addLog('info','Lernpunkt: '+(p.content||''));break;case'error':this.addLog('error','Fehler: '+(p.error||''));break;default:this.addLog('info',evt+(p&&Object.keys(p).length?': '+JSON.stringify(p).substring(0,80):''));}if(['cycle_start','cycle_end','task_complete','review_complete','plan_complete'].includes(evt)){this.loadState();this.loadCosts();}},
